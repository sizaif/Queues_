...
private:
     ...
     */
    CMessageQueue(BYTE *pCurrAddr,eQueueModel module, key_t shmKey, int shmId,size_t size,bool isInit);
public:
    ~CMessageQueue();
...
public:
    //创建共享内存
    static BYTE *CreateShareMem(std::string role,key_t iKey, long vSize,enShmModule &shmModule,int& shmId);
    //销毁共享内存
     ...
     * @return
     */
    static CMessageQueue *CreateInstance(std::string role,key_t shmkey,
                                         size_t queuesize,
