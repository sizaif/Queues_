...
namespace shmmqueue
{
CMessageQueue::CMessageQueue(BYTE *pCurrAddr, eQueueModel module, key_t shmKey, int shmId, size_t size,bool isInit)
{
    ...
    if(isInit){
        ...
        // 从共享内存中获取 stMemTrunk
        m_stMemTrunk = reinterpret_cast<stMemTrunk*>(m_pQueueAddr);
        m_pQueueAddr += sizeof(stMemTrunk);
    }
...
{
    printf("Mem trunk address %p,shmkey %d ,shmid %d, size %d, begin %d, end %d, queue module %d \n",
           m_stMemTrunk,
    ...
    printf("Mem sizeof trunk: %p , %lu , %p, quaddr: %p \n",m_stMemTrunk,sizeof(m_stMemTrunk),m_stMemTrunk+sizeof(m_stMemTrunk),m_pQueueAddr);
}
...
/**
  ...
  *返回值         ： 共享内存块地址
**/
BYTE *CMessageQueue::CreateShareMem(std::string role,key_t iKey, long vSize, enShmModule &shmModule,int& shmId)
{
    size_t iTempShmSize;
    if (iKey < 0) {
    ...
    printf("Try to malloc share memory of %zu bytes... with role: [ %s ] \n", iTempShmSize,role.c_str());
    ...
    if (shmId < 0) {
        ...
        if(role == "READ"){
            printf("[READ] Same shm seg [key= %d] exist, now delete it and create new \n", iKey);
            //先获取之前的shmId
            ...
            printf("[WRITE] Same shm seg [key= %d] exist, now try to attach it... \n", iKey);
    ...
    printf("Successfully alloced share memory block,[key= %d,shmId %d] size = %zu \n", iKey, shmId, iTempShmSize);
...
CMessageQueue *CMessageQueue::CreateInstance(std::string role,key_t shmkey,
                                             size_t queuesize,
                                             eQueueModel queueModule)
{
    ...
    int shmId = 0;
    BYTE * tmpMem = CMessageQueue::CreateShareMem(role,shmkey, queuesize + sizeof(stMemTrunk), shmModule,shmId);
    CMessageQueue *messageQueue = nullptr;
    if(shmModule == enShmModule::SHM_INIT){
        // 初始化stMemTrunk
        messageQueue= new CMessageQueue(tmpMem,queueModule, shmkey,shmId, queuesize,true);
    }else if(shmModule == enShmModule::SHM_RESUME){
        // 从共享内存中获得stMemTrunk
        messageQueue = new CMessageQueue(tmpMem,queueModule, shmkey,shmId, queuesize,false);
    }
